...
class FollowerListFragment : Fragment() {
    ...
    inner class FollowerListFragmentRecyclerViewAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
            ...
            firestore?.collection("follows")?.document(selectedUid!!)?.get()?.addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    if(followerDTO == null){
                        ...
                        getPhotos(followerDTO.followers)
                    }
                }
            }
        }
        ...
        fun getPhotos(followers: MutableMap<String, Boolean>) {

            followerListenerRegistration = firestore?.collection("images")?.orderBy("timestamp")?.addSnapshotListener { querySnapshot, firebaseFirestoreException ->
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            */
            ...
            // 이메일 아이디 가져오기
            viewHolder.follower_list_email.text = photoDTOs!![position].userEmail
